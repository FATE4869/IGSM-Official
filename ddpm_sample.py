...
if __name__ == "__main__":
    ...
    if accelerator.is_main_process:
        ...
        elif 'celeba' in args.model_path:
            example_inputs = {'sample': torch.randn(1, 3, 64, 64).to(accelerator.device), 'timestep': torch.ones((1,)).long().to(accelerator.device)}
        else:
    ...
    with torch.no_grad():
        ...
        for i in tqdm(range(num_batches), disable=not accelerator.is_main_process):
            outputs = pipeline(batch_size=args.batch_size, num_inference_steps=args.ddim_steps, generator=generator)
