...
class ResnetBlock2D(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        if non_linearity == "swish":
            ...
            self.nonlinearity = nn.functional.silu()
        ...
        if self.up:
            if kernel == "fir":
                ...
                self.upsample = self.upsample_2d
            elif kernel == "sde_vp":
            ...
            if kernel == "fir":
                self.fir_kernel = (1, 3, 3, 1)
                self.downsample = self.downsample_2d
            elif kernel == "sde_vp":
    ...
    def upsample_2d(self, x, factor=2, gain=1):
        assert isinstance(factor, int) and factor >= 1
    ...
    def downsample_2d(self, x, factor=2, gain=1):
        ...
        pad_value = kernel.shape[0] - factor
        output = upfirdn2d_native(
            x, kernel.to(device=x.device), down=factor,
            pad=((pad_value + 1) // 2, pad_value // 2)
        )
        return output
