...
class ScoreSdeVeScheduler(SchedulerMixin, ConfigMixin):
    ...
    def step_pred(
        ...
        delta_t = torch.tensor(1 / self.config.num_train_timesteps)
        # caleb: we will use the ftle_features to for knowledge distillation
        dx = - drift + diffusion * noise
        ftle_features = - drift + diffusion * noise * torch.sqrt(delta_t)
        if not return_dict:
            return (prev_sample, prev_sample_mean, dx)

        output = SdeVeOutput(prev_sample=prev_sample, prev_sample_mean=prev_sample_mean)
        output.dx = dx
        return output
