from diffusers import DiffusionPipeline, DDPMPipeline, DDIMPipeline, DDIMScheduler, DDPMScheduler, UNet2DModel
...
parser.add_argument("--pruner", type=str, default='taylor',
                    choices=['taylor', 'random', 'magnitude', 'reinit', 'lamp', 'BNScale', 'absolute', 'diff-pruning'])
...
if __name__ == '__main__':
    ...
    if args.pruner in ['taylor', 'diff-pruning', 'BNScale']:
    ...
    if args.pruning_ratio > 0.0:
        ...
        elif args.pruner == 'lamp':
            imp = tp.importance.LAMPImportance()
        elif args.pruner == 'BNScale':
        ...
        print("#Basic Params: {:.4f} M".format(base_params / 1e6))
        print("# Basic MACS: {:.4f} G".format(base_macs / 1e9))
        model.zero_grad()
        model.eval()
        import random
